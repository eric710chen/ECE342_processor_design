...
always_comb begin
	...
	o_remaining_attempts = 3'b111;
	
	case (state)
		S_Random: begin
			if (reset) nextstate = S_Random;
			else if (!remaining_attempts) nextstate = S_Random;
			else if (i_enter) begin
				if (o_over) nextstate = S_Greater;
				else if (o_under) nextstate = S_Smaller;
				else if (o_equal) nextstate = S_Equal;
				o_update_leds = 1'b1;
				o_update_leds = 1'b0;
				o_remaining_attpempts = remaining_attempts - 1'b1;
			end
		end
